...
import isi.deso.tpds.MetodoPago.FactoryPago;
...
public class VentanaDeCreacionEdicionPedido extends javax.swing.JFrame {
    ...
    private void botonAceptarActionPerformed(java.awt.event.ActionEvent evt) throws SQLException {//GEN-FIRST:event_botonAceptarActionPerformed
        ...
        if (filaSeleccionada == 100) {
            Pedido pedidoActual = pedidoController.crearRetornarPedido(clienteSeleccionado, pagoPedido, itemsAsignados, Double.parseDouble(campoMontoBase.getText()), Double.parseDouble(campoMontoTotal.getText()));
            pedidoController.modificarPedido(pedidoActual.getId(), clienteSeleccionado, pagoPedido, itemsAsignados, Double.parseDouble(campoMontoBase.getText()), Double.parseDouble(campoMontoTotal.getText()));
            JOptionPane.showMessageDialog(null, "Pedido creado con éxito", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        } else {
            pedidoController.modificarPedido(pedidoActual.getId(), clienteSeleccionado, pagoPedido, itemsAsignados, Double.parseDouble(campoMontoBase.getText()), Double.parseDouble(campoMontoTotal.getText()));
            //pedidoController.actualizarPedido(pedidoActual);
...
//GEN-LAST:event_botonAceptarActionPerformed
    ...
    public void recibirDatosEdicion(int filaSeleccionada, Pedido pedido) {
        ...
        if (pedidoActual.getMetodoPago() != null) {
            String metodoPago = pedidoActual.getMetodoPago().getClass().getSimpleName();
            comboBoxMetodoDePago.setSelectedItem(metodoPago);
        }
        itemsAsignados = pedidoActual.getItemsPedido();
        clienteSeleccionado = pedidoActual.getCliente();
